<template>
  ...
  <div :class="[$style.root, startLoading && $style['root--active']]">

    <span>{{ isLoaded ? 'Done' : 'loading' }}</span>
...
export default Vue.extend({
  ...
  created() {
    this.$watch('startLoading', () => {
      if(this.startLoading) this.addPercentValue()
    })
  },
  computed: {
    isLoaded(): boolean {
  ...
  methods: {
    ...
    addPercentValue() {
      if(!this.startLoading) this.percentage = 0

      const interval = setInterval(() => {
        if (this.percentage < 100) {
          this.percentage += this.randomFromInterval(0.04, 0.3)
        } else {
