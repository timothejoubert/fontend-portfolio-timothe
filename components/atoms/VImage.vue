...
export type VImageProps = Props &
  InterventionRequestProps &
  Omit<NuxtImageProps, 'src'>
...
export default Vue.extend<any, any, any, VImageProps>({
  ...
  render(createElement): VNode {
    const src =
      this.src ||
      this.image?.thumbnail?.relativePath ||
    ...
    else if (this.image?.imageHeight)
      height = parseFloat(this.image.imageHeight)

    // image
    const modifiersKeys = Object.keys(interventionRequestProps).filter(
      (key) => !['width', 'height'].includes(key)
    )
    const imgModifiers = { ...(this.modifiers || {}) }
    modifiersKeys.forEach((key) => {
      if (typeof this.$props[key] !== 'undefined')
        imgModifiers[key] = this.$props[key]
    })
    ...
    const img = createElement(
      this.tag === 'img' || !processable ? 'nuxt-img' : 'nuxt-picture',
      {
        props: imgProps,
        on: {
          load: () => {
    ...
    if (this.ratio || this.placeholder) {
